...
class HarpsN(Instrument):
    ...
    def param_override(self):
        ...
        self.param_set('INSTRUMENT', 'HARPSN', source=func_name)
        # add instrument earth location
        #    (for use in astropy.coordinates.EarthLocation)
        self.param_set('EARTH_LOCATION', 'Roque de los Muchachos')
        ...
        self.param_set('COMPIL_WAVE_MIN', 400, source=func_name)
        ...
        self.param_set('KW_WAVECOEFFS', 'HIERARCH TNG DRS CAL TH COEFF LL{0}',
                        source=func_name)
        # define wave num orders key in header
        self.param_set('KW_WAVEORDN', 'HIERARCH TNG DRS CAL TH ORDER NBR',
                        source=func_name)
        # define wave degree key in header
        self.param_set('KW_WAVEDEGN', 'HIERARCH TNG DRS CAL TH DEG LL',
                        source=func_name)
        # define the key that gives the mid exposure time in MJD
        self.param_set('KW_MID_EXP_TIME', 'HIERARCH TNG DRS BJD',
                        source=func_name)
        # define the start time of the observation
        self.param_set('KW_MJDATE', 'HIERARCH TNG DRS BJD', source=func_name)
        # define snr keyword
        self.param_set('KW_SNR', 'HIERARCH TNG DRS SPE EXT SN47',
                        source=func_name)
        # define berv keyword
        self.param_set('KW_BERV', 'HIERARCH TNG DRS BERV', source=func_name)
        # define the Blaze calibration file
        self.param_set('KW_BLAZE_FILE', 'HIERARCH TNG DRS BLAZE FILE',
                        source=func_name)
        # define the exposure time of the observation
        self.param_set('KW_EXPTIME', 'HIERARCH TNG DET WIN1 DIT1',
                        source=func_name)
        # define the airmass of the observation
        self.param_set('KW_AIRMASS', 'AIRMASS',
                        source=func_name)
        ...
        self.param_set('KW_DPRTYPE', 'HIERARCH TNG DPR TYPE',
                        source=func_name)
        # define the filename of the wave solution
        self.param_set('KW_WAVEFILE', 'HIERARCH TNG DRS CAL TH FILE',
                        source=func_name)
        # define the original object name
        self.param_set('KW_OBJNAME', 'HIERARCH TNG OBS TARG NAME',
                        source=func_name)
        ...
        self.param_set('KW_EXT_SNR', 'HIERARCH TNG DRS SPE EXT SN47',
                        source=func_name)
        # define the barycentric julian date
        self.param_set('KW_BJD', 'HIERARCH TNG DRS BJD', source=func_name)
        # define the reference header key (must also be in rdb table) to
        #    distinguish FP calibration files from FP simultaneous files
        self.param_set('KW_REF_KEY', 'HIERARCH TNG DPR TYPE', source=func_name)
...
class HarpsN_ESO(HarpsN):
    ...
    def param_override(self):
        ...
        # ---------------------------------------------------------------------
        # define the key that gives the mid exposure time in MJD
        self.param_set('KW_MID_EXP_TIME', 'HIERARCH TNG QC BJD',
                        source=func_name)
        # define the start time of the observation
        self.param_set('KW_MJDATE', 'HIERARCH TNG QC BJD', source=func_name)
        # define snr keyword
        self.param_set('KW_SNR', 'HIERARCH TNG QC ORDER65 SNR',
                        source=func_name)
        # define berv keyword
        self.param_set('KW_BERV', 'HIERARCH TNG QC BERV', source=func_name)
        # define the Blaze calibration file
        ...
        # define the exposure time of the observation
        self.param_set('KW_EXPTIME', 'HIERARCH TNG QC BJD',
                        source=func_name)
        # define the airmass of the observation
        self.param_set('KW_AIRMASS', 'AIRMASS',
                        source=func_name)
        ...
        # define the SNR in chosen order
        self.param_set('KW_EXT_SNR', 'HIERARCH TNG QC ORDER65 SNR',
                        source=func_name)
        # define the barycentric julian date
        self.param_set('KW_BJD', 'HIERARCH TNG QC BJD', source=func_name)
        # define the reference header key (must also be in rdb table) to
        #    distinguish FP calibration files from FP simultaneous files
        self.param_set('KW_REF_KEY', 'HIERARCH TNG DPR TYPE', source=func_name)
        # velocity of template from CCF
...
# =============================================================================
# Define HARPS-N ESSP class
# =============================================================================
class HarpsN_ESSP(essp.ESSP):
    def __init__(self, params: base_classes.ParamDict,
                 args: base_classes.ParamDict, name: str = None):
        # get the name
        if name is None:
        ...
        super().__init__(params, args, name)
        # extra parameters (specific to instrument)
        self.default_template_name = 'LBL_Template_{0}_harpsn_essp.fits'
        self.default_mask_name = 'LBL_Mask_{obj}_{mtype}_harpsn_essp.fits'
        self.default_sample_wave_name = 'sample_wave_grid_harpsn_essp.fits'
        # define wave limits in nm
    ...
    def param_override(self):
        ...
        """
        # set function name
        func_name = __NAME__ + '.HarpsN_ESSP.param_override()'
        # first run the inherited method
        super().param_override()
        # ---------------------------------------------------------------------
        # set parameters to update
