...
from lbl.core import fast_lowpass
...
def lowpassfilter(input_vect: np.ndarray, width: int = 101,
    ...
    :return:
    """
    # fast path (numba kernel fast_lowpass.lowpass_windows, same results)
    #   for float64 vectors and integer widths
    if _lowpass_fast_ok(input_vect, width):
        xmed, ymed = fast_lowpass.lowpass_windows(
...
def _lowpass_fast_ok(input_vect: np.ndarray, width: Any) -> bool:
    """
    Whether lowpassfilter can use the numba window loop: fast kernels on
    (npreplica.use_fast), a 1D float64 vector (any byte order) and an
    integer width with a non-zero step
    """
