...
def __main__(inst: InstrumentsType, **kwargs):
    ...
    # the parallel mode needs the fast line kernel (it reports which lines
    #   each file sets, see compute_parallel)
    parallel = nproc > 1 and not multi_mode and recipe_kwargs is not None
    parallel &= npreplica.use_fast()
    if parallel:
...
def worker_main(task_file: str):
    ...
    inst = select.load_instrument(args, plogger=log)
    npreplica.set_fast_kernels(inst.params['FAST_KERNELS'])
    if not npreplica.use_fast():
        raise LblException('lbl_compute worker: fast kernels not available')
